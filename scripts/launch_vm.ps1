param (
    [string]$adminUsername = "fakeUsername",
    [string]$adminPassword = "fakePassword",
    [string]$vmName = "fakeVMName",
    [string]$vmLocation = "fakeLocation",
    [string]$vmSize = "fakeSize",
    [string]$imgName = "fakeImageName"
)

$currentWorkingDir = Split-Path -Parent $MyInvocation.MyCommand.Definition
. $currentWorkingDir\env.ps1

$global:location = $vmLocation
...
function setup_and_deploy_vm (
    ...
    if ($? -eq $False) {
        ...
        $urlOfUploadedImageVhd = "https://tyachenrocketstorage.blob.core.windows.net/rocket-vhds/" + $imgName
...
setup_and_deploy_vm $adminUsername $adminPassword $vmName "testpc" $vmSize $imgName
